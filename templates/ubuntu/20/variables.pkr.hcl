...
variable "vsphere_tools_upgrade_policy" {
  type        = bool
...
variable "vsphere_template_conversion" {
  type        = bool
...
variable "iso_file" {
  type        = string
  description = "The file name of the ISO image. (e.g. 'iso-linux-ubuntu-server-20-04-lts.iso')"
  default     = null
}
...
variable "vsphere_http_port_min" {
  type        = number
  description = "The start of the HTTP port range."
}

variable "vsphere_http_port_max" {
  type        = number
...
variable "vsphere_ip_wait_timeout" {
  type        = string
